...
export const renderTasks = (taskArray, shortcut) => {
  ...
  taskArray.forEach((task) => {
    const markup = `
        <div class="tasks underline-gray ${persistDone(
          task.isDone
        )}" data-taskid="${task.id}" data-projectname="${task.projectName}">
            <div class="tasks-left ${persistLine(task.isDone)}">

                <div class="pretty p-default p-round">
                    <input type="checkbox" class="checkbox" ${persistCheck(
                      task.isDone
                    )}>
                ...
                <p class="task-name shortcut-text ${setFontColor(
                  task.priority
                )}">${task.title}</p>
                <div class="time-remaining ${persistLine(
                  task.isDone
                )}">Due ${calculateTime(task.dueDate)}</div>
            </div>
